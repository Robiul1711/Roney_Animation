...
const ContactMutation=useMutation({
  mutationFn: (data) => axiosPublic.post("/api/contact", data),
  onSuccess: (response) => {
  ...
  return (
    <div className="section-padding-x py-16 " ref={containerRef}>
      <Title level="title32" className="text-white text-center">
        Tell us where you're at
      </Title>


      <div className="my-16 max-w-5xl mx-auto relative">
      <Link to="/">
        <RiCloseLargeFill className="absolute -top-10 right-0 text-3xl cursor-pointer text-white hover:text-custom-primary duration-300" />
      </Link>
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-10" onFocus={() => setOpenDropdown(null)}>
          ...
          <div className="relative">
            <DropdownInput
              ...
              selectedValue={watch("servicesInterested")}
              setSelectedValue={(value) => setValue("servicesInterested", value)}
              isOpen={openDropdown === "service"}
              setIsOpen={() => toggleDropdown("service")}
              options={serviceOptions}
            />
            {errors.servicesInterested && <p className="text-red-400 mt-1 text-sm">Please select at least one service</p>}
          </div>
        ...
        <div className="flex justify-end mt-5">
          <button
            type="submit"
            className="bg-custom-primary hover:bg-green-600 text-white rounded-full py-3 px-6 flex items-center gap-2 hover:bg-opacity-90 transition-colors duration-300"
