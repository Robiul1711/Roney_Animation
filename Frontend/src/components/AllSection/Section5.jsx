    ...
    >
      ...
      <div className="md:mt-12 mt-5 grid grid-cols-2  md:grid-cols-3 gap-4 section-padding-x">
        {data.map((item, index) => (
          <div
            key={item.id}
            className="p-4 md:p-6 border-2 border-custom-primary bg-custom2 text-white  backdrop-blur-xl rounded-2xl flex flex-col gap-5 md:gap-10 items-center justify-center"
          >
            <Title
              level="title48"
              className="text-custom-primary border-b-2 border-white/50 w-full text-center  md:pb-4"
            >
              {typeof counts[index] === "number"
                ? counts[index] + (item.title.includes("+") ? "+" : "")
                : counts[index]}
            </Title>
            <Title level="title20" className="text-white font-semibold text-center">
              {item.desc}
            </Title>
          </div>
        ))}
